...
WxEmitResult createDialog(const RcDialog& dlg)
{
    WxEmitResult out;
...
std::map<std::string, WxEmitResult> WxEmitter::emit(const RcFile& rc) const
{
    std::map<std::string, WxEmitResult> results;
    for (const auto& dlg : rc.dialogs) {
        results[dlg.name] = createDialog(dlg);
    }
